 ...
 */
const SMART_COMMENT_PROMPT = `Ты генерируешь комментарии к постам в Telegram-каналах.

ШАГ 1: Определи язык поста по тексту.
ШАГ 2: Напиши короткий вопрос по теме на том же языке.
...
export function buildBusinessPrompt(_postContent: IPostContent): string {
    ...
    parts.push('\n--- ЗАДАЧА ---');
    parts.push('Ответь ТОЛЬКО вопросом на языке поста. До 80 символов.');
