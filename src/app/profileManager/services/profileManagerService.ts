...
export class ProfileManagerService {
  ...
  async updateUsernameAsync(
    _request: IUsernameUpdateRequest,
    _options?: IProfileManagerOptions,
  ): Promise<IProfileUpdateResult> {
    ...
    try {
      ...
      console.log(
        `📊 [${operationId}] Сессия: ${maskSessionString(_request.sessionString)}`,
      );
      console.log(
        `👤 [${operationId}] Новый username: @${_request.newUsername}`,
      );
      ...
      console.log(
        `📋 [${operationId}] Шаг 2: Проверка доступности username...`,
      );
      const isAvailable = await this.checkUsernameAvailabilityAsync(
        normalizedUsername,
        _request.sessionString,
      );
      ...
      console.log(
        `📋 [${operationId}] Шаг 4: Получение обновленной информации о профиле...`,
      );
      const updatedProfile = await this.getProfileInfoAsync(
        _request.sessionString,
      );
      ...
      if (client) {
        try {
          ...
          console.warn(
            `⚠️ [${operationId}] Предупреждение при отключении клиента:`,
            disconnectError,
          );
        }
      }
      ...
      console.error(
        `❌ [${operationId}] Ошибка при обновлении username:`,
        errorMessage,
      );
  ...
  async updateProfileAsync(
    _request: IProfileUpdateRequest,
    _options?: IProfileManagerOptions,
  ): Promise<IProfileUpdateResult> {
    const operationId = generateOperationId();
    let client: TelegramClient | null = null;
    ...
    try {
      ...
      const errorMessage = formatTelegramError(error);
      console.error(
        `❌ [${operationId}] Ошибка при обновлении профиля:`,
        errorMessage,
      );
   ...
   */
  async updateProfilePhotoAsync(
    _request: IProfilePhotoUpdateRequest,
    _options?: IProfileManagerOptions,
  ): Promise<IProfileUpdateResult> {
    const operationId = generateOperationId();
    let client: TelegramClient | null = null;
    ...
    try {
      ...
      console.log(
        `📸 [${operationId}] Файл изображения: ${_request.photoPath}`,
      );
      ...
      const errorMessage = formatTelegramError(error);
      console.error(
        `❌ [${operationId}] Ошибка при обновлении фото профиля:`,
        errorMessage,
      );
    ...
    try {
      ...
      if (client) {
        try {
          ...
          console.warn(
            "Предупреждение при отключении клиента:",
            disconnectError,
          );
        }
      }
      ...
      throw new Error(
        `Ошибка получения информации о профиле: ${formatTelegramError(error)}`,
      );
    }
  }
   ...
   * Проверка доступности username
   */
  async checkUsernameAvailabilityAsync(
    _username: string,
    ...
    try {
      ...
      console.warn(
        `Ошибка при проверке доступности username: ${formatTelegramError(error)}`,
      );
      return false;
    }
  }
  ...
  async setup2FAAsync(
    ...
    try {
      ...
      console.log(
        `🔐 [${operationId}] Пароль: ${mask2FAPassword(_request.password)}`,
      );
      ...
      const { newAlgo, newPasswordHash } = this.createNewPasswordAlgoAndHash(
        _request.password,
      ...
      const errorMessage = formatTelegramError(error);
      console.error(
        `❌ [${operationId}] Ошибка при установке пароля 2FA:`,
        errorMessage,
      );
   ...
   */
  async change2FAAsync(
    _request: I2FAChangeRequest,
    _options?: IProfileManagerOptions,
  ): Promise<IProfileUpdateResult> {
    const operationId = generateOperationId();
    let client: TelegramClient | null = null;
    ...
    try {
      ...
      const srpPassword = await this.createSrpPasswordAsync(
        client,
      ...
      const errorMessage = formatTelegramError(error);
      console.error(
        `❌ [${operationId}] Ошибка при изменении пароля 2FA:`,
        errorMessage,
      );
  ...
  async get2FAStatusAsync(_sessionString: string): Promise<I2FAStatus> {
    ...
    try {
      ...
      throw new Error(
        `Ошибка получения статуса 2FA: ${formatTelegramError(error)}`,
      );
    }
  }
  ...
  async disable2FAAsync(
    ...
    try {
      ...
      console.log(
        `📊 [${operationId}] Сессия: ${maskSessionString(_sessionString)}`,
      );
      ...
      const errorMessage = formatTelegramError(error);
      console.error(
        `❌ [${operationId}] Ошибка при отключении 2FA:`,
        errorMessage,
      );
   ...
   */
  private async createSrpPasswordAsync(
    ...
    try {
      ...
      throw new Error(
        `Ошибка создания SRP пароля: ${error instanceof Error ? error.message : "Неизвестная ошибка"}`,
      );
    }
  }
   ...
   */
  private createNewPasswordAlgoAndHash(
    _password: string,
    _algo: any,
  ): { newAlgo: any; newPasswordHash: Buffer } {
    try {
      // Согласно документации Telegram API
      const baseAlgo =
        _algo as Api.PasswordKdfAlgoSHA256SHA256PBKDF2HMACSHA512iter100000SHA256ModPow;
