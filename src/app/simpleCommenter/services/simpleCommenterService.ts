...
import {
  ...
  delayAsync,
  cleanUsername,
} from "../parts/simpleCommenterHelpers";
...
export class SimpleCommenterService implements ISimpleCommenter {
  ...
  async postCommentsAsync(
    _options: ISimpleCommentOptions,
  ): Promise<ISimpleCommentResponse> {
    ...
    if (_options.useAI) {
      ...
      throw new Error(
        "Базовые комментарии удалены. Используйте AI режим (useAI: true)",
      );
    }
      ...
      for (const [index, channelUsername] of _options.channels.entries()) {
        ...
        try {
          ...
          if (result.error && result.error.includes("Flood wait")) {
            ...
            console.log(
              `🛡️ FloodWait обнаружен, увеличиваю задержки до ${this.p_adaptiveDelay}мс`,
            );
          ...
          if (result.success) {
            ...
            console.log(
              `✅ [${index + 1}/${_options.channels.length}] ${aiTag} @${cleanedUsername} - Успешно`,
            );
            console.log(`💬 "${result.commentText}"`);
          } else {
            failedComments++;
            console.log(
              `❌ [${index + 1}/${_options.channels.length}] @${cleanedUsername} - ${result.error}`,
            );
          }
        } catch (error) {
          failedComments++;
          ...
          if (isCriticalFloodWait(error)) {
            ...
            console.error(
              `📊 Успешно обработано: ${successfulComments}/${index + 1} каналов`,
            );
          ...
          console.log(
            `❌ [${index + 1}/${_options.channels.length}] @${cleanedUsername} - ${errorMessage}`,
          );
        ...
        if (index < _options.channels.length - 1) {
          const totalDelay = Math.max(
            _options.delayBetweenComments,
            this.p_adaptiveDelay,
          );
          console.log(
            `⏱️ Ожидание ${totalDelay / 1000}с до следующего канала...`,
          );
    ...
    console.log(`\n🏁 === ИТОГИ СЕССИИ ===`);
    console.log(
      `📊 Успешно: ${successfulComments}/${_options.channels.length}`,
    );
    console.log(`❌ Ошибок: ${failedComments}/${_options.channels.length}`);
    console.log(
      `🤖 AI запросов: ${totalAIRequests} (успешно: ${successfulAIRequests})`,
    );
  ...
  private async postCommentToChannelAsync(
    ...
    try {
      ...
      const postContent = await extractPostContent(
        this.p_client,
        _channelUsername,
      );
      ...
      if (!shouldComment.shouldComment) {
        throw new Error(
          `Пост не подходит для комментирования: ${shouldComment.reason}`,
        );
      }
      ...
      if (this.p_aiGenerator) {
        ...
        const aiResult =
          await this.p_aiGenerator.generateCommentAsync(postContent);
        usedAI = true;
      ...
      if (
        ...
        discussionResult.messages.length === 0
      ) {
      ...
      if (_sendAsChannelId) {
        ...
        const sendResult = await this.p_client.invoke(
          new Api.messages.SendMessage({
            ...
            sendAs: new Api.InputPeerChannel({
      ...
      if (isCriticalFloodWait(error)) {
        console.error(
          `🛑 Критический FloodWait в канале @${_channelUsername}!`,
        );
