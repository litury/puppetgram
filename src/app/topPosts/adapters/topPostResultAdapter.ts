...
import { ITopPostAnalysisResponse, IChannelTopPostsResult, IPostAnalysisResult } from '../interfaces';
...
export class TopPostResultAdapter {
     ...
     */
    private static formatPostResult(post: IPostAnalysisResult, rank?: number): string {
        ...
        const previewText = msg.message.length > 150
    ...
    static generateSummary(result: ITopPostAnalysisResponse): string {
        const totalViews = result.overallTopPosts.reduce((sum, post) => sum + post.metrics.views, 0);
        const totalForwards = result.overallTopPosts.reduce((sum, post) => sum + post.metrics.forwards, 0);
        const totalReactions = result.overallTopPosts.reduce((sum, post) => sum + post.metrics.reactions, 0);

        const avgEngagement = result.overallTopPosts.length > 0
            ? result.overallTopPosts.reduce((sum, post) => sum + post.metrics.engagement, 0) / result.overallTopPosts.length
            : 0;
