...
export class TopPostAnalyzerService implements ITopPostAnalyzer {
     ...
     */
    async analyzeTopPostsAsync(options: ITopPostAnalysisOptions): Promise<ITopPostAnalysisResponse> {
        console.log(`🔍 Начинаю анализ топ постов для ${options.channels.length} каналов...`);
        ...
        for (const channelName of options.channels) {
            try {
                ...
                console.error(`❌ Ошибка анализа канала ${channelName}:`, error.message);
        ...
        const sortedAllPosts = this.sortPostResults(allTopPosts, options.criteria.sortBy);
        ...
        console.log(`✅ Анализ завершен! Обработано ${totalMessagesAnalyzed} сообщений из ${channelResults.length} каналов`);
    ...
    async analyzeChannelPostsAsync(
        ...
        const postResults: IPostAnalysisResult[] = filteredMessages.map(message => {
     ...
     */
    private async fetchChannelMessages(channelName: string, limit: number): Promise<IMessage[]> {
    ...
    private convertTelegramMessage(msg: any): IMessage {
        ...
        const hasMedia = !!(msg.media && msg.media.className !== 'MessageMediaEmpty');
    ...
    calculateEngagement(message: IMessage): number {
        ...
        const reactions = (typeof message.reactions === 'number') ? message.reactions : 0;
        ...
        const engagementScore = (forwards * 3 + reactions * 2 + replies * 4) / views * 100;
    ...
    filterMessages(messages: IMessage[], criteria: IPostAnalysisCriteria): IMessage[] {
        return messages.filter(message => {
            ...
            if (criteria.minViews && (message.views || 0) < criteria.minViews) return false;
            if (criteria.minForwards && (message.forwards || 0) < criteria.minForwards) return false;
            if (criteria.minReactions && (typeof message.reactions === 'number') && message.reactions < criteria.minReactions) return false;
            ...
            if (criteria.minTextLength && textLength < criteria.minTextLength) return false;
            if (criteria.maxTextLength && textLength > criteria.maxTextLength) return false;
    ...
    sortMessages(messages: IMessage[], sortBy: IPostAnalysisCriteria['sortBy']): IMessage[] {
        return [...messages].sort((a, b) => {
            switch (sortBy) {
                ...
                case 'combined':
                    return this.calculateCombinedScore(b) - this.calculateCombinedScore(a);
                default:
                    return (b.engagement || 0) - (a.engagement || 0);
            }
        });
     ...
     */
    private sortPostResults(results: IPostAnalysisResult[], sortBy: IPostAnalysisCriteria['sortBy']): IPostAnalysisResult[] {
     ...
     */
    private calculateScore(message: IMessage, sortBy: IPostAnalysisCriteria['sortBy']): number {
        switch (sortBy) {
            case 'views':
    ...
    private calculateCombinedScore(message: IMessage): number {
        ...
        return views * 0.2 + forwards * 0.3 + reactions * 0.2 + replies * 0.2 + engagement * 0.1;
    }
