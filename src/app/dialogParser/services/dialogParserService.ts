...
export class DialogParserService implements IDialogParser {
   ...
   */
  async parseUserDialogsAsync(
    _options: IDialogParseOptions,
  ): Promise<IDialogParseResult> {
    ...
    try {
      ...
      const exportPath = path.join(
        this.p_exportDir,
        `user_${targetUserId}_${sessionId}`,
      );
   ...
   */
  async parseChatUsersAsync(
    _options: IChatParseOptions,
  ): Promise<IChatUsersParseResult> {
    ...
    try {
      ...
      for (const message of allMessages) {
        ...
        if (
          !userStats[userId].firstMessageDate ||
          message.date < userStats[userId].firstMessageDate!
        ) {
          userStats[userId].firstMessageDate = message.date;
        }

        if (
          !userStats[userId].lastMessageDate ||
          message.date > userStats[userId].lastMessageDate!
        ) {
      ...
      const filteredUsers = Object.values(userStats).filter((user) => {
        if (
          _options.minMessagesPerUser &&
          user.messageCount < _options.minMessagesPerUser
        ) {
          return false;
        }
      ...
      const exportPath = path.join(
        this.p_exportDir,
        `chat_${_options.chatId}_${sessionId}`,
      );
   ...
   */
  async parseUserInChatAsync(
    _options: IUserInChatParseOptions,
  ): Promise<IUserInChatParseResult> {
    const sessionId = await generateDialogSessionId();
    const startTime = new Date();
    ...
    try {
      ...
      for (const [userId, userInfo] of userInfoMap) {
        ...
        if (_options.targetUsername && userInfo.username) {
          const cleanUsername = _options.targetUsername
            .replace("@", "")
            .toLowerCase();
      ...
      const exportPath = path.join(
        this.p_exportDir,
        `user_${targetUser.id}_chat_${_options.chatId}_${sessionId}`,
      );
  ...
  async exportChatUsersAsync(
    ...
    const { ChatUsersExportAdapter } = await import(
      "../adapters/chatUsersExportAdapter"
    );
      ...
      chats: [
        {
          ...
          lastMessageDate:
            _result.messages.length > 0
              ? _result.messages[_result.messages.length - 1].date
              : new Date(),
        },
      ],
      ...
      while (true) {
        ...
        for (const message of batch) {
          if (message.message && message.senderId) {
            const userMessage = await this.convertMessageToUserMessage(
              message,
              _chatId,
              _options,
            );
   ...
   */
  private filterChatsByOptions(
    _chats: IChatInfo[],
    _options: IDialogParseOptions,
  ): IChatInfo[] {
    return _chats.filter((chat) => {
      // Фильтр по типам чатов
      if (!_options.includePrivateChats && chat.type === "private")
        return false;
      if (!_options.includeGroups && chat.type === "group") return false;
      if (!_options.includeSupergroups && chat.type === "supergroup")
        return false;
  ...
  private messageMatchesFilters(
    ...
    // Фильтр по длине
    if (
      _filters.minMessageLength &&
      _message.text.length < _filters.minMessageLength
    )
      return false;
  ...
  private filterMessages(
    ...
    return _messages.filter((message) =>
      this.messageMatchesFilters(message, _filters),
    );
  }
   ...
   */
  private calculateDateRange(_messages: IUserMessage[]): {
    from: Date;
    to: Date;
  } {
  ...
  calculateMessageStats(_messages: IUserMessage[]): IUserMessageStats {
    ...
    const averageMessageLength =
      _messages.reduce((sum, m) => sum + m.text.length, 0) / totalMessages;
    ...
    return {
      ...
      mostActiveChat:
        _messages.find((m) => m.chatId === mostActiveChat)?.chatTitle || "",
      mostActivePeriod,
      messagesByHour,
      messagesByDay,
      messagesByChat,
