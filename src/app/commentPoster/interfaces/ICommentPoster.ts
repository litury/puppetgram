...
import { IAICommentGenerator, IAICommentResult } from '../../aiCommentGenerator/interfaces';
...
export interface IPostContent {
    ...
    reactions: number;
    hasMedia: boolean;
    mediaType?: 'photo' | 'video' | 'document' | 'audio' | 'sticker' | 'voice' | 'animation' | 'poll' | 'contact' | 'location';
    channelId: string;
    channelUsername: string;
    channelTitle: string;
...
export interface ICommentPoster {
     ...
     */
    postCommentsAsync(_options: ICommentingOptionsWithCache): Promise<ICommentingResponseWithContent>;

    /**
     * Комментирование с AI генерацией
     */
    postCommentsWithAIAsync(_options: ICommentingOptionsWithAI): Promise<ICommentingResponseWithAI>;

    /**
     * Тестирование извлечения контента без комментирования
     */
    testContentExtractionAsync(_options: IContentExtractionTestOptions): Promise<IContentExtractionTestResult>;

    /**
     * Проверка доступности каналов для комментирования
     */
    checkChannelAccessAsync(_targets: ICommentTarget[]): Promise<IChannelFilteringResponse>;
