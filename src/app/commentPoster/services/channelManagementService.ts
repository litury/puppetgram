...
export class ChannelManagementService {
    ...
    if (result.error) {
      ...
      if (errorLower.includes("flood") || errorLower.includes("wait")) {
        return {
          ...
          reason: "flood_wait",
          retryAfter: new Date(
            Date.now() + options.retryIntervalHours * 60 * 60 * 1000,
          ),
        };
      }
  ...
  private async executeActions(
    ...
    if (channelsToRemove.length > 0) {
      ...
      console.log(
        `✅ Удалено ${channelsToRemove.length} каналов из исходного JSON`,
      );
    }

    // 2. Создаем архивную базу
    if (channelsToArchive.length > 0) {
      const archiveFile = this.createArchiveDatabase(
        channelsToArchive,
        options,
      );
      ...
      console.log(
        `📦 Создана архивная база: ${archiveFile} (${channelsToArchive.length} каналов)`,
      );
    }
    ...
    if (channelsToRetry.length > 0 && options.createRetryDatabase) {
      ...
      console.log(
        `🔄 Создана база для повтора: ${retryFile} (${channelsToRetry.length} каналов)`,
      );
    }
  ...
  private createRetryDatabase(
    ...
    const retryData: IProgressFileData = {
      ...
      lastProcessedChannel:
        channels[channels.length - 1]?.channelUsername || "",
  ...
  private async loadOriginalJson(
    filePath: string,
  ): Promise<IProgressFileData | null> {
    try {
  ...
  private convertToChannelWithCache(
    originalChannel: any,
  ): ICommentTargetWithCache {
    return {
      channelId: originalChannel.channel.channelId,
  ...
  private calculateSummary(actions: IChannelManagementAction[]): {
    ...
    const successCount = actions.filter(
      (a) => a.reason === "successful_comment",
    ).length;

    return {
      totalProcessed: actions.length,
      successRate:
        actions.length > 0 ? (successCount / actions.length) * 100 : 0,
      byAction,
      byReason,
    };
  }
  ...
  private logResults(
    result: IChannelManagementResult,
    options: IChannelManagementOptions,
  ): void {
