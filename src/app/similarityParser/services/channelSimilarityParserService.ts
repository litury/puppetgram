...
import {
  handleFloodWaitError,
  isCriticalFloodWait,
} from "../../../shared/utils/floodWaitHandler";
import { SimilarityResultAdapter } from "../adapters";
import {
  loadExclusionFilters,
  filterDuplicateChannels,
  addToExclusionList,
} from "../utils/duplicateFilter";
...
class TelegramApiAdapter implements ITelegramApiAdapter {
  ...
  async getChannelRecommendationsAsync(
    _channelName: string,
    _limit: number,
  ): Promise<any> {
    try {
      ...
      if (!inputChannel)
        throw new Error(`Канал ${_channelName} не найден или недоступен`);
      const { Api } = await import("telegram");
      return await this.p_telegramClient.invoke(
        new Api.channels.GetChannelRecommendations({ channel: inputChannel }),
      );
    } catch (error: any) {
      if (isCriticalFloodWait(error))
        handleFloodWaitError(
          error,
          `Получение рекомендаций для ${_channelName}`,
        );
  ...
  async resolveChannelAsync(_channelName: string): Promise<any | null> {
    try {
      ...
      const { Api } = await import("telegram");
      const result = await this.p_telegramClient.invoke(
        new Api.contacts.ResolveUsername({ username: normalizedName }),
      );
      ...
      if (isCriticalFloodWait(error))
        handleFloodWaitError(error, `Резолв канала ${_channelName}`);
...
export class ChannelSimilarityParserService {
  ...
  async parseSimilarChannelsAsync(
    _options: ISimilarityParsingOptions,
  ): Promise<ISimilarityParsingResult> {
    ...
    try {
      ...
      console.log(
        `📋 Фильтр: ${validatedOptions.minSubscribers}-${validatedOptions.maxSubscribers} подписчиков`,
      );
      ...
      while (
        context.foundChannels.size < validatedOptions.targetChannelCount &&
        queue.length > 0
      ) {
        console.log(
        ...
        context.depthStatistics[currentLevel] = {
          channelsFound: 0,
          channelsProcessed: 0,
          sourceChannels: [],
        };
        ...
        for (const sourceChannel of queue) {
          if (context.foundChannels.size >= validatedOptions.targetChannelCount)
            break;
          ...
          context.depthStatistics[currentLevel].sourceChannels.push(
            sourceChannel,
          );
          ...
          try {
            ...
            for (const channel of recommended) {
              if (channel.username && !context.foundChannels.has(channel.id)) {
                ...
                if (
                  subscribers >= validatedOptions.minSubscribers &&
                  subscribers <= validatedOptions.maxSubscribers
                ) {
                  context.foundChannels.set(channel.id, channel);
                  context.depthStatistics[currentLevel].channelsFound++;
                  newChannelsFromSource++;
                  if (
                    context.foundChannels.size >=
                    validatedOptions.targetChannelCount
                  )
                    break;
                }
              }
            ...
            console.log(
              `   - Из @${normalizeChannelName(sourceChannel)}: найдено ${newChannelsFromSource} новых релевантных каналов`,
            );
            ...
            console.error(
              `   - ❌ Ошибка при обработке @${normalizeChannelName(sourceChannel)}: ${error.message}`,
            );
      ...
      if (finalChannels.length > validatedOptions.targetChannelCount) {
        finalChannels = finalChannels.slice(
          0,
          validatedOptions.targetChannelCount,
        );
      }
      ...
      const { filteredChannels, excludedCount } = filterDuplicateChannels(
        finalChannels,
        exclusionFilters.excludeUsernames,
      );
      if (excludedCount > 0) {
        console.log(
          `🚫 Исключено ${excludedCount} каналов из предыдущих поисков.`,
        );
      ...
      if (validatedOptions.removeDuplicates) {
        ...
        duplicatesRemoved = calculateDuplicatesRemoved(
          originalCount,
      ...
      const result: ISimilarityParsingResult = {
        ...
        targetReached:
          finalChannels.length >= validatedOptions.targetChannelCount,
      };
  ...
  private async performBasicSearch(
    ...
    try {
      const apiResult = await this.p_apiAdapter.getChannelRecommendationsAsync(
        _channelName,
        _limit,
      );
  ...
  async validateChannelAccessAsync(_channelName: string): Promise<boolean> {
    try {
      return (
        (await this.p_apiAdapter.resolveChannelAsync(_channelName)) !== null
      );
    } catch (error) {
      return false;
    }
  ...
  private async autoExportResults(
    _result: ISimilarityParsingResult,
  ): Promise<void> {
    try {
  ...
  private async savePartialResults(
    ...
    try {
      ...
      const txtContent =
        SimilarityResultAdapter.exportChannelNames(partialResult);
      ...
      console.error(
        `❌ Ошибка сохранения частичных результатов:`,
        error.message,
      );
    }
  }
