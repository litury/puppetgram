...
const log = createLogger("AutoFilterGroups");
...
class AutoGroupFilter {
  ...
  async run(): Promise<void> {
    log.info("🤖 АВТОМАТИЧЕСКАЯ AI-ФИЛЬТРАЦИЯ ГРУПП И ЧАТОВ");
    ...
    try {
      ...
      log.warn(
        "Группы с политическим, военным, запрещенным контентом будут покинуты!",
      );
  ...
  private async loadGroups(): Promise<void> {
    log.info("📋 ЗАГРУЗКА ГРУПП И ЧАТОВ");

    log.info("Сканирую участие в группах и чатах...");
    ...
    try {
      ...
      for await (const dialog of this.client.iterDialogs()) {
        ...
        if (entity) {
          if (entity.className === "User") {
            ...
            if (groupCount % 25 === 0) {
              log.info(`   👥 Найдено групп/чатов: ${groupCount}`);
            }

            // Останавливаемся если достигли лимита
            if (groupCount >= maxGroups) {
              log.warn(`Достигнут лимит ${maxGroups} групп`);
              break;
            }
          }
        }
        ...
        if (dialogCount % 200 === 0) {
          log.info(
            `Просканировано: ${dialogCount} (каналы: ${channelCount}, группы: ${groupCount}, чаты: ${chatCount})`,
          );
        }
      ...
      log.info(`   📺 Пропущено каналов: ${channelCount}`);
      log.info(`   👥 Найдено групп: ${groupCount}`);
      log.info(`   💬 Пропущено личных чатов: ${chatCount}`);

      if (this.stats.total === 0) {
        log.warn("❌ Нет групп для анализа");
        log.info(
          "💡 Убедитесь что выбранный аккаунт состоит в группах или чатах",
        );
        process.exit(0);
      }
    } catch (error: any) {
      if (error.errorMessage === "FLOOD_WAIT") {
        log.error(
          `⏱️ FloodWait: подождите ${error.seconds} секунд и попробуйте снова`,
        );
      } else {
  ...
  private isGroupOrChat(dialog: any): boolean {
    const entity = dialog.entity;
    return entity && entity.className === "Channel" && !entity.broadcast; // Группы это каналы без broadcast
  }

  private async analyzeGroups(): Promise<void> {
    log.info("🔍 ПАКЕТНЫЙ АНАЛИЗ ГРУПП");
    ...
    log.info(
      `Пакетная обработка: ${groupsToAnalyze.length} групп в ${totalBatches} пакетах по ${batchSize}`,
    );
    ...
    for (let batchIndex = 0; batchIndex < totalBatches; batchIndex++) {
      ...
      const batchPromises = batch.map((group, index) =>
        this.analyzeGroupInBatch(
          group,
          startIndex + index + 1,
          groupsToAnalyze.length,
        ),
      );
    ...
    log.info(`🚪 Покинул: ${this.stats.leftGroups} групп`);
  }

  /**
   * Обрабатывает одну группу в пакете
   */
  private async analyzeGroupInBatch(
    group: any,
    groupNumber: number,
    totalGroups: number,
  ): Promise<any> {
    ...
    try {
      ...
      if (messages.length === 0) {
        log.info(
          `[${groupNumber}/${totalGroups}] ⏭️ ${groupName} - пропущена (нет сообщений)`,
        );
        this.stats.analyzed++;
        return null;
      }
      ...
      if (shouldLeave) {
        log.error(
          `[${groupNumber}/${totalGroups}] ❌ ${groupName} - фильтры: ${reasons.join("; ")}`,
        );
      } else {
        log.info(
          `[${groupNumber}/${totalGroups}] ✅ ${groupName} - прошла все фильтры`,
        );
      }
      ...
      log.error(
        `[${groupNumber}/${totalGroups}] ⚠️ Ошибка анализа ${groupName}: ${error}`,
      );
      this.stats.errors++;
      return null;
    }
  ...
  private async leaveGroup(group: any, reasons: string[]): Promise<void> {
    ...
    try {
      ...
      log.info(`  🚪 Покинул группу ${groupName}`);
      log.info(`     Причина: ${reasons.join("; ")}`);

      // Пауза после выхода из группы
      await new Promise((resolve) => setTimeout(resolve, 1000));
    } catch (error) {
      log.error(`  ❌ Не удалось покинуть группу ${groupName}: ${error}`);
      this.stats.errors++;
    }
  }
