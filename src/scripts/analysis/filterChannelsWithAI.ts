...
class ChannelAIFilter {
  ...
  private readonly FILTERS: FilterConfig[] = [
    {
      ...
      enabled: false,
      systemPrompt:
        'Ты эксперт по анализу контента. Определи является ли канал политическим. Отвечай только "ДА" или "НЕТ" и кратко объясни почему.',
      userPrompt: (posts) =>
        `Проанализируй эти посты и определи является ли канал политическим:\n\n${posts.join("\n---\n")}`,
    },
    {
      name: "Каналы на нецелевом языке",
      enabled: false,
      systemPrompt:
        'Ты лингвист. Определи является ли основной язык канала нецелевым. Отвечай только "ДА" или "НЕТ" и кратко объясни.',
      userPrompt: (posts) =>
        `Определи язык этих постов (целевой или нет):\n\n${posts.join("\n---\n")}`,
    },
    {
      name: "Каналы про военные конфликты/войну",
      enabled: false,
      systemPrompt:
        'Ты контент-аналитик. Определи посвящен ли канал военной тематике, военные конфликты или военным новостям. Отвечай только "ДА" или "НЕТ" и кратко объясни.',
      userPrompt: (posts) =>
        `Определи является ли канал военным/про военные конфликты:\n\n${posts.join("\n---\n")}`,
    },
  ];
  ...
  private isChannel(dialog: any): boolean {
    ...
    return (
      entity && entity.className === "Channel" && entity.broadcast === true
    );
  }
  ...
  private async analyzeChannel(dialog: any): Promise<ChannelAnalysis> {
    ...
    for (const filter of this.selectedFilters) {
      ...
      if (result.matched) {
        ...
        if (filter.name.includes("нецелевом языке"))
          analysis.isUkrainian = true;
        if (filter.name.includes("военные конфликты"))
          analysis.isWarRelated = true;
      }
    }
  ...
  private async getChannelPosts(
    ...
    try {
      const messages = await this.gramClient
        .getClient()
  ...
  private displayResults(): void {
    ...
    const toUnsubscribe = this.analysisResults.filter(
      (r) => r.shouldUnsubscribe,
    );
  ...
  private async exportResults(): Promise<void> {
    ...
    const exportData = {
      ...
      toUnsubscribe: this.analysisResults.filter((r) => r.shouldUnsubscribe)
        .length,
