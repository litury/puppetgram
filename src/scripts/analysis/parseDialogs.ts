...
} from "../../app/dialogParser/interfaces";
...
async function main() {
  ...
  try {
    ...
    console.log(
      `👤 Текущий пользователь: ${fullName} (@${currentUser.username}, ID: ${currentUser.id})\n`,
    );
    ...
    console.log("\n📋 === НАСТРОЙКИ ЭКСПОРТА ===");
    console.log(
      `📅 Период: ${options.filters.dateFrom?.toLocaleDateString() || "начало"} - ${options.filters.dateTo?.toLocaleDateString() || "сегодня"}`,
    );
    console.log(
      `💬 Мин. длина сообщения: ${options.filters.minMessageLength || "любая"}`,
    );
    ...
    console.log(
      `📅 Период: ${result.dateRange.from.toLocaleDateString()} - ${result.dateRange.to.toLocaleDateString()}`,
    );
 ...
 */
async function createCustomExportOptions(
  _parserService: DialogParserService,
): Promise<IDialogParseOptions> {
  ...
  const limitStr = await question(
    "Максимум сообщений (Enter = без ограничений): ",
  );
...
async function createDateFilters(): Promise<Partial<IDialogFilters>> {
  ...
  const dateFromStr = await question(
    "Дата начала (YYYY-MM-DD, Enter = с начала): ",
  );
  const dateToStr = await question(
    "Дата окончания (YYYY-MM-DD, Enter = до сегодня): ",
  );
...
async function createAdditionalFilters(): Promise<Partial<IDialogFilters>> {
  ...
  const minLengthStr = await question(
    "Минимальная длина сообщения (Enter = любая): ",
  );
...
async function createExportConfig(): Promise<IDialogExportConfig> {
  ...
  const sortOrder: "asc" | "desc" =
    orderChoice.toLowerCase() === "desc" ? "desc" : "asc";
      ...
      if (typeChats.length > 0) {
        ...
        typeChats.slice(0, 10).forEach((chat) => {
          const lastActivity = chat.lastMessageDate
            ...
            : "неизвестно";
          console.log(
            `  • ${chat.title} ${chat.username ? `(@${chat.username})` : ""} - ${lastActivity}`,
          );
        });
...
async function parseChatUsers(
  ...
  const minMessagesStr = await question(
    "Минимум сообщений от пользователя (Enter = 1): ",
  );
  ...
  const exportByUsers = await question(
    "Создавать отдельные файлы для каждого пользователя? (y/n): ",
  );
  ...
  console.log(
    `📅 Период: ${filters.dateFrom?.toLocaleDateString() || "начало"} - ${filters.dateTo?.toLocaleDateString() || "сегодня"}`,
  );
  console.log(
    `💬 Мин. длина сообщения: ${filters.minMessageLength || "любая"}`,
  );
  console.log(
    `👥 Мин. сообщений от пользователя: ${options.minMessagesPerUser}`,
  );
 ...
 */
async function parseUserInChat(
  _parserService: DialogParserService,
): Promise<void> {
  ...
  switch (searchType) {
    case "1":
      ...
      break;
    case "2":
      targetUsername = await question(
        "Введите username (без @, например: username): ",
      );
      if (!targetUsername) {
  ...
  try {
    ...
    console.log(`💬 Найдено сообщений: ${result.totalMessages}`);
    console.log(
      `👤 Пользователь: ${result.targetUser.fullName} (@${result.targetUser.username || "no_username"})`,
    );
    console.log(
      `📅 Период: ${result.dateRange.from.toLocaleDateString()} - ${result.dateRange.to.toLocaleDateString()}`,
    );
