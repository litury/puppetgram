...
import {
  TopPostAnalyzerService,
  TopPostResultAdapter,
} from "../../app/topPosts";
import {
  IPostAnalysisCriteria,
  ITopPostAnalysisOptions,
} from "../../app/topPosts/interfaces";
import { ExportService } from "../../shared/exportService";
import { loadChannelsFromFile } from "../../shared/utils/helpers";

async function main() {
  console.log(
    "🔍 Добро пожаловать в анализатор лучших постов Telegram каналов!",
  );
...
async function handleTopPostAnalysis(
  ...
  const channelsInput = await prompts({
    ...
    validate: (value) =>
      value.length > 0 ? true : "Необходимо указать хотя бы один канал",
  });
